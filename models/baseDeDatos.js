...
db.serialize(() => {
  ...
  db.run(`
    ...
    CREATE TABLE IF NOT EXISTS calificacion(
      ...
      puntos INTEGER NOT NULL, FOREIGN KEY(producto_id) REFERENCES productos(producto_id))`)
...
function detalles(req, res) {
  const id = req.params.id;
  const sql = `SELECT * FROM imagenes WHERE productoID = ?`;
  db.all(sql, [id], (err, rowsImagenes) => {
    console.log(rowsImagenes, "datos recibidos de la base de datos ");
    if (err) {
      console.error(err.message);
      res.status(500).send('Error en el servidor');
      return;
...
async function ClientesGET(req, res) {
  const sql = `SELECT p.*, AVG(calificacion.puntos) AS puntos, i.url AS imagen_url
  FROM productos p
  LEFT JOIN imagenes i ON p.producto_id = i.productoID
  LEFT JOIN calificacion ON p.producto_id = calificacion.producto_id
...
async function comprarPOST(req, res) {
  ...
  let data;
  const { nombre_de_usuario, apellido, cedula, nombre_producto, cliente_id, producto_id, cantidad, total_pagado, codigo, ip_cliente, numeroT, cvv, mesV, year, currency, descripcion, puntos } = req.body;
  const paymentData = {
    "amount": total_pagado,
    "card-number": numeroT,
    "cvv": cvv,
  ...
  const sql = `INSERT INTO compras(nombre_de_usuario,apellido,cedula,nombre_producto,cliente_id,producto_id,cantidad,total_pagado,ip_cliente,codigo,numeroT,cvv,mesV,year,currency,descripcion) 
  VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;
  const sqlcalificacion = "INSERT INTO calificacion(producto_id,user_id,puntos) VALUES(?,?,?)";
  const sqlEmail = "SELECT * FROM usuarios WHERE id = ?";

  db.run(sql, [nombre_de_usuario, apellido, cedula, nombre_producto, cliente_id, producto_id, cantidad, total_pagado, ip_cliente, codigo, numeroT, cvv, mesV, year, currency, descripcion], (err) => {
    if (err) return console.error(err.message);


    db.get(sqlEmail, [cliente_id], (err, rows) => {
      const transporter = nodemailer.createTransport({
        service: 'outlook',
        port: 587,
        tls: {
      ...
      db.run(sqlcalificacion, [producto_id, cliente_id, puntos], (err, rows) => {
